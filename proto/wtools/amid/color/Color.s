...
function _hwbStrToRgb( dst, src )
{
  ...
  if( !_.color._validateHwb( hwbColors ) )
  return null;

  return _.color._hwbLongToRgbVector( dst, hwbColors );
}

//

function _hwbStrToRgba( dst, src )
{
  /* hwb(H, W, B) */
  ...
  return _.color._hwbLongToRgbaVector( dst, hwbColors );
